...
(function () {
  ...
  function today() {
    var dayName = Utils.getDayOfWeek();
    ...
    return '' +
      '<div class="qtime-header">' +
        '<div class="qtime-summary" data-action="toggle-qtime" aria-expanded="false" aria-controls="' + id + '">' +
          ...
          '<span class="qtime-day">' + Utils.esc(Utils.getDayOfWeek().charAt(0).toUpperCase() + Utils.getDayOfWeek().slice(1)) + '</span>' +
        '</div>' +
  ...
  function renderPrintHeader() {
    var domain = today();
    var dayName = Utils.getDayOfWeek();
    var dateStr = Utils.formatDateFull(Utils.isoDate(new Date()));
